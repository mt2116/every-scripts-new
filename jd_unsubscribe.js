...
let goodPageSize = $.getdata('jdUnsubscribePageSize') || 30 // 运行一次取消多少个已关注的商品。数字0表示不取关任何商品
let shopPageSize = $.getdata('jdUnsubscribeShopPageSize') || 30 // 运行一次取消多少个已关注的店铺。数字0表示不取关任何店铺
...
async function jdUnsubscribe(doubleKey) {
    await Promise.all([unsubscribeGoods(doubleKey), unsubscribeShops()])
    await Promise.all([getFollowShops(), getFollowGoods()])
}
function showMsg() {
            ...
            if (goodPageSize * 1 !== 0) {
                if (followGoods.totalNum > 0) {
                    for (let item of followGoods.data) {
                        console.log(
                            `是否匹配：：${item.commTitle.indexOf(
                                stopGoods.replace(/\ufffc|\s*/g, '')
                            )}`
                        )

                        if (
                            stopGoods &&
                            item.commTitle.indexOf(stopGoods.replace(/\ufffc|\s*/g, '')) > -1
                        ) {
    ...
    return new Promise(resolve => {
        const option = {
            ...
            headers: {
                ...
                Referer:
                    'https://wqs.jd.com/my/fav/goods_fav.shtml?ptag=37146.4.1&sceneval=2&jxsid=15963530166144677970',
                ...
                if (followShops.totalNum > 0) {
                    for (let item of followShops.data) {
                        if (
                            stopShop &&
                            item.shopName &&
                            item.shopName.indexOf(stopShop.replace(/\s*/g, '')) > -1
                        ) {
                            console.log(
                                `匹配到了您设定的店铺--${item.shopName}，不在进行取消关注店铺`
                            )
                            break
                        }
    ...
    return new Promise(resolve => {
        const option = {
            ...
            headers: {
                ...
                'Accept-Language': 'zh-cn',
                Referer:
                    'https://wqs.jd.com/my/fav/shop_fav.shtml?sceneval=2&jxsid=15963530166144677970&ptag=7155.1.9',
                'Accept-Encoding': 'gzip, deflate, br'
            }
        }
        $.get(option, (err, resp, data) => {
        ...
        const option = {
            ...
            headers: {
                ...
                'User-Agent': $.isNode()
                    ...
                    : 'jdapp;iPhone;9.2.2;14.2;%E4%BA%AC%E4%B8%9C/9.2.2 CFNetwork/1206 Darwin/20.1.0',
                Referer:
                    'https://wqs.jd.com/my/fav/shop_fav.shtml?sceneval=2&jxsid=15960121319555534107&ptag=7155.1.9',
                Cookie: cookie,
                'Accept-Language': 'zh-cn',
                'Accept-Encoding': 'gzip, deflate, br'
            }
